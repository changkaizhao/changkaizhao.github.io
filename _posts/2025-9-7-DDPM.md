...
* **DDPM 的网络本质在学习各噪声层级下的“分数函数（score）”**

  $$
  s_\theta(x_t,t)\;\approx\;\nabla_{x_t}\log q_t(x_t),
  $$

  也就是对当前噪声尺度 $$t$$ 的**对数密度对 $$x_t$$ 的梯度**（一个向量场）。

* 之所以很多实现“看起来不是在学梯度”，而是在做 **MSE 回归**（学 $$\varepsilon$$ 或 $$x_0$$ 或 $$v$$），是因为在**高斯腐蚀**下它们与 score 存在**一一线性对应**，所以“回归这些量”与“学习 score”是等价的（即**去噪分数匹配**）：

  1. 传统 **$$\varepsilon$$-prediction**
  ...
  2. **$$x_0$$-prediction**
     ...
     $$
     s_\theta(x_t,t)= -\,\frac{x_t-\sqrt{\bar\alpha_t}\,x_{0,\theta}(x_t,t)}{\,1-\bar\alpha_t\,}.
     $$

  3. **$$v$$-prediction**（$$v=\sqrt{\bar\alpha_t}\varepsilon-\sqrt{1-\bar\alpha_t}\,x_0$$）
...
因此：**把 DDPM 的神经网络理解为“在每个噪声层级上学习一个求梯度(score)的函数”是正确的**；只是实现上通常以回归 $$\varepsilon/x_0/v$$ 的 **MSE** 来训练，因为它们和 score 等价、数值更稳定。
